...
VigenereAnalyser::VigenereAnalyser() {
...
*    Date: 31/05/2011
*    Code version: 1.0
*    Availability: http://rosettacode.org/wiki/Vigen%C3%A8re_cipher/Cryptanalysis#C.2B.2B
*	 Usage: Used with slight changes ( Due to incorrect approach as can be seen in rot13.decodeSecretFile(); )
***************************************************************************************/
...
void VigenereAnalyser::decypherSecret() {
	...
	pair<string, string> output = va.analyze(fileContents);

	cout << "Key: " << output.second << endl << endl;
	cout << "Text: " << output.first << endl;

	ofstream outputSecret;
	outputSecret.open("decodedSecret.txt");
	outputSecret << output.first;
}